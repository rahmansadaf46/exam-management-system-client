...
const AddMarkViva = ({ modalIsOpen, closeModal, resultSheet,  modalData, setResultSheet }) => {
    ...
    const onSubmit = data => {
        ...
        console.log(data)
        let mark = data.mark;
        if (mark > modalData.fullMark) {
            window.alert(`Mark is bigger than ${modalData.fullMark}`)
        }
        else {
        ...
        >
            ...
            <form className="p-2" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="form-group">
                    ...
                    <input type="text" ref={register({ required: true })} defaultValue={modalData.obtainedMark} name='mark' className="form-control" />
                    {errors.examName && <span className="text-danger">This field is required</span>}

                </div>
                <div className="form-group text-center">
                    <span className='text-primary'>Total Mark: </span> <span className='text-danger'>{modalData.fullMark}</span>
                </div>
                {/* <div className='row'>
